...
def res_to_format(res):
    ...
    return (f"bestvideo[height<={res}][ext=mp4]+bestaudio[ext=m4a]"
...
def api_files():
    cfg = load_cfg(); folder = cfg["output_dir"]
    ...
    for f in sorted(os.listdir(folder), key=lambda x: os.path.getmtime(os.path.join(folder, x)), reverse=True):
        ...
        if os.path.isfile(fp):
            files.append({"name":f,"size":fmt_size(os.path.getsize(fp)),
...
<div id="app">
  ...
  <nav class="nav">
    ...
    <button class="nav-btn" onclick="switchPanel('files',this)">
      <span class="nav-icon">🗂️</span>ไฟล์
    </button>
  ...
  <div class="panel" id="panel-files">
    <div class="card">
      <div class="section-title">🗂️ จัดการไฟล์</div>
      <div style="margin-bottom:12px">
        <button class="btn btn-dim" onclick="loadFiles()">🔄 รีเฟรช</button>
      </div>
...
async function loadFiles(){
  ...
  if(!files.length){ el.innerHTML='<div class="empty"><div class="empty-icon">📁</div>ไม่มีไฟล์</div>'; return; }
  el.innerHTML = files.map(f=>`
    <div class="list-item">
      <div class="item-icon">🎬</div>
      <div class="item-info">
        <div class="item-title">${esc(f.name)}</div>
        <div class="item-meta">${f.size} | ${f.modified}</div>
      </div>
      <div class="item-actions">
        </a>
        <button class="btn-icon danger" onclick="deleteFile('${esc(f.name)}')" title="ลบ">🗑</button>
      </div>
    </div>`).join('');
}
